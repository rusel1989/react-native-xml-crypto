...
SignedXml.prototype.computeSignature = function(xml, opts) {
  ...
  if (opts.async && typeof this.getAsyncSigner !== 'function') {
    throw new Error("when using 'async' option, you must specify 'getAsyncSigner' instance method")
  }
  ...
  if (opts.async) {
    var self = this;
    return this.createSignatureAsync.bind(this)(signedInfo, prefix)
    .then(function (sig) {
      self.signatureXml += sig
      self.signatureXml += self.getKeyInfo(prefix)
      self.signatureXml += "</" + currentPrefix + "Signature>"
      self.insertSignature(doc, location)
    ...
    this.insertSignature(doc, location)
  }
}

SignedXml.prototype.insertSignature = function(doc, location) {
...
SignedXml.prototype.createSignature = function(signedInfo, prefix) {
  ...
  var canAlgorithm = this.findCanonicalizationAlgorithm(this.canonicalizationAlgorithm)
...
SignedXml.prototype.createSignatureAsync = function(signedInfo, prefix) {
  var xmlNsAttr = 'xmlns'
  ...
  return this.getAsyncSigner(canonizedSignedInfo)
  .then((signature) => {
      this.signatureValue = signature
      return "<" + prefix + "SignatureValue>" + signature + "</" + prefix + "SignatureValue>"
  })
}
